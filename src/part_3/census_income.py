...
df_predictions = lr_model.transform(df_test_transformed)

df_predictions.select("income", "label", "prediction", "probability").show()
...
print(f"Accuracy:  {evaluator_acc.evaluate(df_predictions):.4f}")
print(f"F1 Score:  {evaluator_f1.evaluate(df_predictions):.4f}")
print(f"Precision: {evaluator_precision.evaluate(df_predictions):.4f}")
print(f"Recall:    {evaluator_recall.evaluate(df_predictions):.4f}")
